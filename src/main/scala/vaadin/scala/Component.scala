...
import internal.ListenersTrait
...
package mixins {
  ...
  trait OverridableAbstractComponent {
    self: com.vaadin.ui.AbstractComponent =>

    var c: AbstractComponent = null

    override def attach(): Unit = c.triggerAttach()

    override def detach() = c.triggerDetach()
  }
...
abstract class AbstractComponent(val p: com.vaadin.ui.AbstractComponent with AbstractComponentMixin) extends Component with Sizeable {
  ...
  val detachListeners = collection.mutable.Set[AbstractLayout => Unit]()
  val attachListeners = collection.mutable.Set[AbstractLayout => Unit]()
}
